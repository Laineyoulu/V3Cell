...
def training_loop(
    ...
    balanced_sampling       = False,    # Sample classes uniformly when labels are present.
    ...
    sampler_kwargs = dict(dataset=training_set, rank=rank, num_replicas=num_gpus, seed=random_seed)
    use_balanced = balanced_sampling and training_set.has_onehot_labels
    if balanced_sampling and (not training_set.has_onehot_labels) and rank == 0:
        print('Balanced sampling requested but dataset has no discrete labels; falling back to default sampler.')
    if rank == 0 and use_balanced:
        print('Using class-balanced sampler (uniform over labels).')
    sampler_cls = misc.BalancedInfiniteSampler if use_balanced else misc.InfiniteSampler
    training_set_sampler = sampler_cls(**sampler_kwargs)
