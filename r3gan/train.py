...
# Prioritise r3gan/ so that `from training import training_loop` resolves to r3gan/training/
_R3GAN_DIR = str(Path(__file__).parent)
if _R3GAN_DIR not in sys.path:
    sys.path.insert(0, _R3GAN_DIR)

# Project root comes second so that torch_utils, dnnlib, and models can be imported directly
...
def launch_training(c, desc, outdir, dry_run):
    ...
    print(f'Balanced sampling:   {getattr(c, "balanced_sampling", False)}')
    print()
...
@click.option('--balanced',     help='Enable class-balanced sampling when labels are present', metavar='BOOL', type=bool, default=False, show_default=True)
...
def main(**kwargs):
    ...
    c.balanced_sampling = opts.balanced
    ...
    # Adjust c_dim based on whether the dataset has labels and conditioning is enabled
    if not opts.cond or not c.training_set_kwargs.use_labels:
        opts.cond = False
        c.G_kwargs.ConditionEmbeddingDimension = 0
        c.D_kwargs.ConditionEmbeddingDimension = 0
    else:
        # Fallback for conditional runs on presets that don't set embedding dims explicitly.
        if 'ConditionEmbeddingDimension' not in c.G_kwargs:
            c.G_kwargs.ConditionEmbeddingDimension = NoiseDimension
        if 'ConditionEmbeddingDimension' not in c.D_kwargs:
            c.D_kwargs.ConditionEmbeddingDimension = WidthPerStage[0]
